...
const Technologies = ({ technologies }) => {
    return <div className='w-full grid_card'>
        {
...
const Slider = ({ projects }) => {
    ...
    const sliderIntervalRef = useRef(null);  // Use useRef to store the interval

    if(projects.length === 0) return null;

    const items = projects.map((project) => {
        return <div key={project.title} className="pt-5 text-center justify-center">
    ...
    // Interval to change the slide automatically every 5 seconds
    ...
    return (
        <div className="relative justify-center mb-5 pb-5 minibackdrop2 pl-10 pr-10 mb-20">
            {items.map((item, index) => (
                <div
                    key={index}
                    className={`transition-opacity duration-300 ${index === currentIndex ? 'opacity-100' : 'opacity-0 pointer-events-none hidden'
                        }`}
                >
                    {item}
                </div>
                ...
                <div>
                    <div className='place-content-center items-center mt-10 h-full w-full'>
                        <div className='p-10'>
                            ...
                            {workSelected?.largeText.map((text) => {
                                return <>
